  ...
  <div @click.self="closeBooking" v-show="showModal" class="modal-shad">
	<div class="book-window">
		...
		<select v-model="roomId" v-if="roomType" id="selector">
			<option v-for="(room, i) in roomType.roomModels" :key="i" :value="room.id">{{ room.roomNumber }} номер ({{ room.bedNumber }} чел)</option>
		</select>
		<span style="display: block; margin-bottom: 1vw;" class="validation"><span v-if="details">{{ details }}</span></span>
		<button class="datepicker-book" @click="booking(roomId, date[0], date[1])">Забронировать</button>
	</div>
  </div>
</template>
